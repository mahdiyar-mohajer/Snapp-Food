...
@section('content')
    <div class="container mx-auto">
        <div class="py-8">
            ...
            <form action="{{ route('resturant.updateProfile') }}" method="POST">
                @csrf

                <div class="mb-4">
                    <label for="name" class="block text-gray-600">Restaurant Name</label>
                    <input type="text" name="name" id="name" class="border rounded px-4 py-2 w-full" value="{{ old('name') }}" required>
                </div>

                <div class="mb-4">
                    <label for="phone_number" class="block text-gray-600">Phone Number</label>
                    <input type="text" name="phone_number" id="phone_number" class="border rounded px-4 py-2 w-full" value="{{ old('phone_number') }}" required>
                </div>

                <div class="mb-4">
                    <label for="start_time" class="block text-gray-600">Opening Time</label>
                    <input type="time" name="start_time" id="start_time" class="border rounded px-4 py-2 w-full" value="{{ old('start_time') }}" required>
                </div>

                <div class="mb-4">
                    <label for="end_time" class="block text-gray-600">Closing Time</label>
                    <input type="time" name="end_time" id="end_time" class="border rounded px-4 py-2 w-full" value="{{ old('end_time') }}" required>
                </div>

                <div class="mb-4">
                    <label for="ship_price" class="block text-gray-600">Shipping Price</label>
                    <input type="number" name="ship_price" id="ship_price" class="border rounded px-4 py-2 w-full" value="{{ old('ship_price') }}" required>
                </div>
